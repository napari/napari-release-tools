...
def main():
    ...
    parser.add_argument(
        '--target-branch', help='The target branch', default=''
    )
...
def filter_pr(
    ...
    consumed_pr = (
        get_consumed_pr(repo, target_branch) if target_branch else set()
    )
    ...
    for pull in sorted(pr_to_list, key=lambda x: x.closed_at):
        print(
            f' * [{"x" if pull.number in consumed_pr else " "}] #{pull.number}'
        )
